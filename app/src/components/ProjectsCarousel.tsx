...
export default function ProjectsCarousel({ projects, intervalMs = 6000 }: ProjectsCarouselProps) {
  ...
  useEffect(() => {
    if (isPaused || prefersReducedMotion.current || projects.length <= 1) return;
    const timer = setInterval(() => {
      setSlide(([current]) => [(current + 1) % projects.length, 1]);
    }, intervalMs);
    return () => clearInterval(timer);
  }, [isPaused, intervalMs, projects.length]);
          ...
          >
            <div className="grid gap-0 lg:grid-cols-[1.1fr_1fr]">
              <div className="relative min-h-[220px] bg-[#0f2233]">
                ...
                />
                <div aria-hidden="true" className="absolute inset-0 bg-gradient-to-t from-[#0b1b2a]/70 via-transparent to-transparent"></div>
              </div>
              ...
              <div className="flex flex-col gap-5 p-6">
                <div className="space-y-2">
                  ...
                  <h3 className="text-2xl font-black tracking-tight text-[#121212] dark:text-[#F7F7F7]">{project.title}</h3>
                  <p className="text-sm leading-7 text-[#4b5563] dark:text-[#d5dde4]">{project.summary}</p>
                </div>

                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-[#0f3347]/70 dark:text-white/60">Highlights</p>
                  <ul className="mt-3 grid gap-2 text-sm text-[#334155] dark:text-[#d5dde4]">
                    {project.highlights.map((highlight) => (
                      <li key={highlight} className="flex items-start gap-2">
                        <span className="mt-2 inline-flex h-2 w-2 rounded-full bg-[#008C9E] dark:bg-[#4CAF50]"></span>
                        <span>{highlight}</span>
                      </li>
                    ))}
                  </ul>
      ...
      {projects.length > 1 && (
        <>
          <button
            ...
            className="absolute left-3 top-1/2 -translate-y-1/2 z-10 flex h-10 w-10 items-center justify-center rounded-full border border-black/10 bg-white/90 text-[#0f3347] shadow-md backdrop-blur transition hover:bg-white dark:border-white/10 dark:bg-[#0f2233]/90 dark:text-white dark:hover:bg-[#0f2233]"
          >
          ...
          <button
            ...
            onClick={() => goTo(index + 1, 1)}
            className="absolute right-3 top-1/2 -translate-y-1/2 z-10 flex h-10 w-10 items-center justify-center rounded-full border border-black/10 bg-white/90 text-[#0f3347] shadow-md backdrop-blur transition hover:bg-white dark:border-white/10 dark:bg-[#0f2233]/90 dark:text-white dark:hover:bg-[#0f2233]"
          >
