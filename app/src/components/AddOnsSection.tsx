...
import type { AddOnCategoryContent, AddOnItemContent } from '../lib/content';

interface AddOnsSectionProps {
  categories: AddOnCategoryContent[];
}
...
function AddonCard({ item }: { item: AddOnItemContent }) {
  const Icon = ICON_MAP[item.iconKey] ?? DEFAULT_ICON;
  return (
    <motion.div
 ...
 */
export default function AddOnsSection({ categories }: AddOnsSectionProps) {
  return (
            ...
            return (
              ...
              >
                <div className="space-y-3">
                  {category.items.map((item) => {
                    const ItemIcon = ICON_MAP[item.iconKey] ?? DEFAULT_ICON;
                    return (
                      <div
                        key={item.id}
