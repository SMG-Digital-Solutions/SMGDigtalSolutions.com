...
/**
 * Segmented tab control with an animated sliding pill behind the active
 * option. Built on ARIA's tablist/tab pattern so the active option is
 * exposed to assistive tech via `aria-selected`, not just color.
 */
          ...
          return (
            <button
              ...
              className={`relative rounded-full px-5 py-2.5 text-sm font-semibold transition-colors ${
                isActive
                  ? 'text-white'
                  : 'text-[#121212]/70 hover:text-[#121212] dark:text-white/70 dark:hover:text-white'
              }`}
            >
              {isActive && (
                <motion.span
